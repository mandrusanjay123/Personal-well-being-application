...
app = Flask(__name__)
app.secret_key = 'your_secret_key'  # Required for flash messaging and sessions
...
def assign_cluster():
    ...
    session['user_id'] = 1  # Use a unique user ID if available, or generate it dynamically if needed
...
def task_page():
    # Redirect to the questionnaire if user_cluster is not set
    user_id = session.get('user_id')
    user_cluster = session.get('user_cluster')
...
def show_schedule():
    ...
    # Generate the schedule using the assigned user cluster
